...
You no longer need to perform: maintaining work-item lifecycle/domain state manually; CTRL-005 supplies the deterministic orchestration boundary, CTRL-006 supplies deterministic CI/evidence classification and retry-request handoff, and CTRL-007 supplies deterministic Architect-review observation plus durable review packets and same-worker/same-PR handoff requests without taking semantic authority or executing worker resumes.
The next automation milestone is: **CTRL-009 — Recovery / idempotency**. CTRL-009 is now explicitly defined, frozen, and activated as the sole READY item; implementation may proceed only from the exact current governance activation base recorded in machine state. Stage 1 remains active until an explicit stage transition is supported by accepted repository evidence.
