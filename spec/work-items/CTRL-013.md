...
Reconciliation on the new repository of record is authorized by the Architect governance dispatch `payswapdotorg/controller` issue #1 (2026-09-05): the authoritative remote is now `payswapdotorg/controller` (a fork snapshot of the prior project history at `cbb40d00c4971d4b8cb9af78d8eb3c4dd179ab99`), the repository identity is re-pinned accordingly, the already-implemented CTRL-013 work is completed per the normal post-merge reconciliation protocol, and exactly one reconciliation PR is produced against the exact current `main` SHA.
...
- Implementation PR: #41 on the prior repository record (branch `ctrl-013-github-browser-app`, base `894b443a8c587d5176659bad4135b319a76bc6fe`); its merge commit `cbb40d00c4971d4b8cb9af78d8eb3c4dd179ab99` and the full governed commit history are present in this repository's history (the fork snapshot preserved history; nothing was reset).
...
```text
PR: #41 (prior repository record; merge present in this repository's history)
...
Repository identity reconciliation, per the Architect governance dispatch (issue #1 on `payswapdotorg/controller`):

- the authoritative `repository` identity is re-pinned from `pectoraux/controller` to `payswapdotorg/controller` (machine state; the continuation aid's repository line; the operator README's clone identity; the real-repository domain test pin) — the single explicitly authorized immutable-field exception;
- `payswapdotorg/controller` is a fork of the prior remote whose `main` is exactly the observed CTRL-013 implementation merge `cbb40d00c4971d4b8cb9af78d8eb3c4dd179ab99`, so the governed history and evidence lineage are preserved with no history reset and no parallel authority introduced;
- controlled-repository target semantics are unchanged: the concrete MVP target repository remains `pectoraux/smallapp` (the Controller repository and the controlled product repository are distinct concepts).
...
No runtime implementation semantics are changed by this reconciliation checkpoint: `controller/` and `extension/` are byte-identical to the observed merge commit (audit-proven).
