...
CTRL-001 is complete and reconciled: accepted and merged via PR #1 and recorded in machine state `completed`. **CTRL-002 — Domain/state model is now defined and frozen in `spec/work-items/CTRL-002.md`, and machine state marks it `READY`; it is the only eligible implementation item.** Later items remain planned until their predecessor is complete and machine state is reconciled. No worker may begin a planned item merely because its code appears useful.
